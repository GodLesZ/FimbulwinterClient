...
namespace FimbulwinterClient.Screens
{
	public class IngameScreen : IGameScreen
	{
		...
		{
			RagnarokClient.Singleton.GraphicsDevice.Clear(ClearOptions.Target | ClearOptions.DepthBuffer, Color.DarkSlateBlue, 1.0f, 0);

			RagnarokClient.Singleton.GraphicsDevice.DepthStencilState = DepthStencilState.Default;
			RagnarokClient.Singleton.GraphicsDevice.BlendState = BlendState.Opaque;
			RagnarokClient.Singleton.GraphicsDevice.RasterizerState = RasterizerState.CullNone;
		...
		{
			if (!RagnarokClient.Singleton.IsActive)
				return;
